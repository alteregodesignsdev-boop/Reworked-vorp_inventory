...
-- Load inventory weapons on client start
	function InventoryService.getLoadout(loadout)
		for _, weapon in ipairs(loadout) do
			...
			local weaponUsed = weapon.used == 1 or weapon.used == true
			local weaponUsed2 = weapon.used2 == 1 or weapon.used2 == true

			if weapon.currInv == "default" and (weapon.dropped == nil or weapon.dropped == 0) then
				local newWeapon = Weapon:New({
					id = tonumber(weapon.id),
					propietary = weapon.propietary or weapon.identifier,
					...
					currInv = weapon.currInv or weapon.curr_inv or "default",
					dropped = weapon.dropped or 0,
					group = 5,
