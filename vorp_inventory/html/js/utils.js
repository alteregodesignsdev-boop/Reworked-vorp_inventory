...
function cacheImage(item) {
    ...
    const key = (image || '').toString().trim();
    const isPathLike = /^url\(/i.test(key) || /^(https?:\/\/|img\/|\.?\/|\/)/i.test(key) || /\.(png|jpg|jpeg|webp|gif)$/i.test(key);
    // If metadata is already a path or has an extension, skip preloading here
...
function getItemTooltipContent(image, groupKey, group, limit, weight, degradation, tooltipData) {
    ...
    const raw = (image || 'default').toString().trim();
    let url;
    if (/^url\(/i.test(raw)) {
        // already formatted as a CSS url()
        url = raw;
    } else if (/\.(png|jpg|jpeg|webp|gif)$/i.test(raw)) {
        // looks like a filename with extension; use it directly or under img/items
        const path = /[\\/]/.test(raw) ? raw : `img/items/${raw}`;
        url = `url("${path}")`;
    } else if (/^(https?:\/\/|img\/|\.?\/|\/)/i.test(raw)) {
        // explicit path without extension handling
        url = `url("${raw}")`;
    } else {
        // name without extension; consult cache or default to img/items/<name>.png
        url = imageCache[raw] || `url("img/items/${raw}.png")`;
    }
