...
function DBService.GetInventoryLayout(identifier, charidentifier)
    local row = DBService.singleAwait("SELECT layout FROM inventory_layouts WHERE identifier = @identifier AND charidentifier = @charidentifier", {
        identifier = tostring(identifier),
        charidentifier = tonumber(charidentifier)
    })
    if row and row.layout then
        return json.decode(row.layout)
    end
    return nil
end

function DBService.SaveInventoryLayout(identifier, charidentifier, layout)
    local exists = DBService.singleAwait("SELECT id FROM inventory_layouts WHERE identifier = @identifier AND charidentifier = @charidentifier", {
        identifier = tostring(identifier),
        charidentifier = tonumber(charidentifier)
    })

    local payload = { identifier = tostring(identifier), charidentifier = tonumber(charidentifier), layout = json.encode(layout) }
    if exists and exists.id then
        DBService.updateAsync("UPDATE inventory_layouts SET layout = @layout, updated_at = CURRENT_TIMESTAMP WHERE id = @id", {
            id = tonumber(exists.id),
            layout = payload.layout
        })
    else
        DBService.insertAsync("INSERT INTO inventory_layouts (identifier, charidentifier, layout) VALUES (@identifier, @charidentifier, @layout)", payload)
    end
end
