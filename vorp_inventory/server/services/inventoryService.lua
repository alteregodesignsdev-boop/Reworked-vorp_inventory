...
local function normalizePositiveInt(value)
...
local function rateLimit(source, key, cooldownMs)
	local now = GetGameTimer()
	local perSource = actionTimestamps[source]
	if not perSource then
		perSource = {}
		actionTimestamps[source] = perSource
	end
	local last = perSource[key] or 0
...
local function isCoordsInRange(source, coords, maxDistance)
	coords = toVector3(coords)
	if not coords then
		return false
	end
	maxDistance = (type(maxDistance) == "number" and maxDistance > 0) and maxDistance or 5.0
	local ok, inRange = pcall(function()
		...
		return #(sourceCoords - coords) <= maxDistance
	end)
	if ok then
		return inRange
	end
...
local function isPlayerInRange(source, target, maxDistance)
	...
	local ok, inRange = pcall(function()
		...
		return #(targetCoords - sourceCoords) <= maxDistance
	end)
	if ok then
		return inRange
	end
...
AddEventHandler("playerDropped", function()
	...
	if _source then
		actionTimestamps[_source] = nil
	end
end)

local function waitForServerItemsReady()
	local tries = 0
	while (type(ServerItems) ~= "table" or next(ServerItems) == nil) and tries < 200 do
		Wait(50)
		tries = tries + 1
	end
	return type(ServerItems) == "table" and next(ServerItems) ~= nil
end
...
function InventoryService.UseItem(data)
	...
	if not rateLimit(_source, "UseItem", Config.SpamDelay or 2000) then
		return
	end
	...
	local itemId <const> = tonumber(data.id)
	local itemName <const> = data.item
	if not itemId or type(itemName) ~= "string" then
		return
	end
...
function InventoryService.DropMoney(amount)
	...
	amount = normalizePositiveInt(amount)
	if not amount then
		return
	end
	if not rateLimit(_source, "DropMoney", 250) then
		return
	end
...
function InventoryService.giveMoneyToPlayer(target, amount)
	...
	if not rateLimit(_source, "GiveMoney", 250) then
		return TriggerClientEvent("vorp_inventory:ProcessingReady", _source)
	end
	if not isPlayerInRange(_source, _target, 5.0) then
		return TriggerClientEvent("vorp_inventory:ProcessingReady", _source)
	end
...
function InventoryService.DropGold(amount)
	...
	if not rateLimit(_source, "DropGold", 250) then
		return
	end
...
function InventoryService.giveGoldToPlayer(target, amount)
	...
	if not rateLimit(_source, "GiveGold", 250) then
		return
	end
	if not isPlayerInRange(_source, target, 5.0) then
		return
	end
...
function InventoryService.subItem(source, invId, itemId, amount)
	...
	local invInfo <const> = CustomInventoryInfos[invId]
	if not invInfo then
		return false
	end
	...
	local userInventory <const> = invInfo.shared and UsersInventories[invId] or UsersInventories[invId][identifier]
...
function InventoryService.onPickup(data)
	...
	if not rateLimit(_source, "PickupItem", 150) then
		return
	end
	local user <const> = Core.getUser(_source)
	...
	if pickupLocks[uid] then
		return
	end
	if not isCoordsInRange(_source, pickup.coords, 5.0) then
		return
	end

	pickupLocks[uid] = true
	SvUtils.ProcessUser(_source)
...
function InventoryService.onPickupMoney(data)
	...
	if type(data) ~= "table" or not data.uuid then
		return
	end
	if not rateLimit(_source, "PickupMoney", 150) then
		return
	end
	data = MoneyPickUps[data.uuid]
	if not data then return end
	if not isCoordsInRange(_source, data.coords, 5.0) then
		return
	end
...
function InventoryService.onPickupGold(data)
	...
	if not rateLimit(_source, "PickupGold", 150) then
		return
	end
...
function InventoryService.sharePickupServerWeapon(data)
	...
	if not rateLimit(_source, "SharePickupWeapon", 250) then
		return
	end
	...
	if not weapon or weapon.propietary ~= identifier then
		return
	end

	local result = InventoryService.subWeapon(_source, weaponId)
	if not result then
		return
	end
...
function InventoryService.sharePickupServerItem(data)
	...
	if not rateLimit(_source, "SharePickupItem", 250) then
		return
	end
	local user = Core.getUser(_source)
	if not user then return end
	...
	local amount = normalizePositiveInt(data.amount)
	...
	if amount > item:getCount() then return end

	local result = InventoryService.subItem(_source, "default", itemId, amount)
...
function InventoryService.shareMoneyPickupServer(data)
	...
	if not rateLimit(_source, "SharePickupMoney", 250) then
		return
	end
	local user = Core.getUser(_source)
	if not user then return end
...
function InventoryService.shareGoldPickupServer(data)
	...
	if not rateLimit(_source, "SharePickupGold", 250) then
		return
	end
	local user = Core.getUser(_source)
	if not user then return end
	...
	if amount > gold then return end

	Character.removeCurrency(1, amount)
	local uid = SvUtils.GenerateUniqueID()
	TriggerClientEvent("vorpInventory:shareGoldPickupClient", -1, data.handle, amount, data.position, uid, 1)
...
function InventoryService.DropWeapon(weaponId)
	...
	if not rateLimit(_source, "DropWeapon", 250) then
		return
	end
...
function InventoryService.DropItem(itemName, itemId, amount, metadata, degradation)
	...
	if not rateLimit(_source, "DropItem", 250) then
		return
	end
	...
	local itemNameSafe = item:getName()
	local metadataSafe = item:getMetadata()
	local degradationSafe = item:getDegradation()
	if not Config.DeleteOnlyDontDrop then
		TriggerClientEvent("vorpInventory:createPickup", _source, itemNameSafe, amount, metadataSafe, 1, itemId, degradationSafe)
	else
...
function InventoryService.GiveWeapon(weaponId, target)
	...
	weaponId = tonumber(weaponId)
	target = tonumber(target)
	if not weaponId or not target then
		return
	end
	if not rateLimit(_source, "GiveWeapon", 250) then
		return
	end
	if not isPlayerInRange(_source, target, 5.0) then
	...
	if not SvUtils.InProcessing(_source) then
		...
		if UsersWeapons.default[weaponId] ~= nil and UsersWeapons.default[weaponId].propietary == identifier then
...
function InventoryService.GiveItem(itemId, amount, target)
	...
	itemId = tonumber(itemId)
	amount = normalizePositiveInt(amount)
	if not itemId or not amount or not _target then
		return
	end
	if not rateLimit(_source, "GiveItem", 250) then
		return
	end
	if not isPlayerInRange(_source, _target, 5.0) then
		return
...
function InventoryService.getItemsTable()
	...
	if waitForServerItemsReady() then
...
function InventoryService.getInventoryForSource(_source)
	local sourceCharacter = Core.getUser(_source)
	if not sourceCharacter then return end

	if not waitForServerItemsReady() then
		return
	end
	...
	if sourceCharId ~= nil then
		...
		for _, weapon in pairs(UsersWeapons.default) do
			if weapon.propietary == sourceIdentifier and weapon.charId == sourceCharId and weapon.currInv == "default" and weapon.dropped == 0 then
				hasWeaponForChar = true
				break
			end
		end
		if not hasWeaponForChar then
			local rows = DBService.queryAwait('SELECT * FROM loadout WHERE charidentifier = ? AND curr_inv = ?', { sourceCharId, "default" }) or {}
			for _, db_weapon in pairs(rows) do
				local dropped = tonumber(db_weapon.dropped) or 0
				if dropped == 0 then
					local ammo = db_weapon.ammo and json.decode(db_weapon.ammo) or {}
					local comp = db_weapon.components and json.decode(db_weapon.components) or {}
					...
					local weapon = Weapon:New({
						...
						ammo = ammo,
						components = comp,
						used = db_weapon.used == 1,
						used2 = db_weapon.used2 == 1,
					...
					UsersWeapons.default[weapon:getId()] = weapon
				end
			end
		end
...
function InventoryService.getInventory()
	InventoryService.getInventoryForSource(source)
end
...
AddEventHandler("onResourceStart", function(resourceName)
	...
	CreateThread(function()
		repeat Wait(500) until waitForServerItemsReady()
		Wait(1000)
		for _, playerId in ipairs(GetPlayers()) do
			InventoryService.getInventoryForSource(tonumber(playerId))
		end
	end)
end)
...
function InventoryService.MoveToCustom(obj)
	local _source = source
	if not rateLimit(_source, "MoveToCustom", 150) then
		return
	end
	local data = safeJsonDecode(obj)
	if type(data) ~= "table" then
		return
	end
	...
	local amount = (item.type == "item_weapon") and 1 or normalizePositiveInt(data.number)
	if not amount then
		return
	end
	...
	if item and item.type ~= "item_weapon" then
		local sourceInventory <const> = UsersInventories.default[sourceIdentifier]
		if not sourceInventory then
			return
		end
		local sourceItem <const> = sourceInventory[item.id]
		...
		item = {
			...
			label = sourceItem.metadata?.label or sourceItem:getLabel(),
			type = sourceItem:getType(),
			count = sourceItem:getCount(),
			metadata = sourceItem:getMetadata(),
			degradation = sourceItem:getDegradation(),
			percentage = sourceItem:getPercentage(),
		}
	end
	...
	if item.type == "item_weapon" then
		...
		InventoryService.addItem(_source, invId, item.name, amount, item.metadata, info, function(itemAdded)
			...
			local removed = InventoryService.subItem(_source, "default", item.id, amount)
			if not removed then
				InventoryService.subItem(_source, invId, itemAdded:getId(), amount)
				SvUtils.Trem(_source)
				return
			end
...
function InventoryService.TakeFromCustom(obj)
	...
	if not rateLimit(_source, "TakeFromCustom", 150) then
		return
	end
	local data = safeJsonDecode(obj)
	if type(data) ~= "table" then
	...
	if item.type == "item_weapon" then
		...
		local invInfo <const> = CustomInventoryInfos[invId]
		local userInventory <const> = invInfo.shared and UsersInventories[invId] or UsersInventories[invId][sourceIdentifier]
		if not userInventory or not item or not item.id then
			SvUtils.Trem(_source)
			return
		end

		local sourceItem <const> = userInventory[item.id]
		if not sourceItem then
			SvUtils.Trem(_source)
			return
		end

		if amount > sourceItem:getCount() then
			SvUtils.Trem(_source)
			return
		end

		local itemName <const> = sourceItem:getName()
		local itemMeta <const> = sourceItem:getMetadata()
		local itemDegradation <const> = sourceItem:getDegradation()
		local itemPercentage <const> = sourceItem:getPercentage()
		local metadataLabel = itemMeta?.label or sourceItem:getLabel()

		local canCarryItem = InventoryAPI.canCarryItem(_source, itemName, amount)
		if not canCarryItem then
			SvUtils.Trem(_source)
			return Core.NotifyObjective(_source, T.cantCarryItemStack, 2000)
		end

		local removed = InventoryService.subItem(_source, invId, sourceItem:getId(), amount)
		if not removed then
			SvUtils.Trem(_source)
			return
		end

		local info = { degradation = itemDegradation, isPickup = false, percentage = itemPercentage }
		InventoryService.addItem(_source, "default", itemName, amount, itemMeta, info, function(itemAdded)
			if not itemAdded then
				InventoryService.addItem(_source, invId, itemName, amount, itemMeta, info, function()
					SvUtils.Trem(_source)
				end)
				return
			end

			TriggerEvent("vorp_inventory:Server:OnItemTakenFromCustomInventory", { id = itemAdded:getId(), name = itemName, amount = amount }, invId, _source)
			TriggerClientEvent("vorpInventory:receiveItem", _source, itemName, itemAdded:getId(), amount, itemAdded:getMetadata(), itemAdded:getDegradation(), itemAdded:getPercentage())
			InventoryService.reloadInventory(_source, invId)
			InventoryService.DiscordLogs(invId, itemName, amount, sourceName, "Take")
...
function InventoryService.MoveToPlayer(obj)
	...
	if not rateLimit(_source, "MoveToPlayer", 150) then
		return
	end
	local data = safeJsonDecode(obj)
	if type(data) ~= "table" then
	...
	local item = data.item
	local target = data.info and data.info.target or nil

	if type(item) ~= "table" then
		return
	end
	local amount = (item.type == "item_weapon") and 1 or normalizePositiveInt(data.number)
	if not amount or not isPlayerInRange(_source, target, 5.0) then
		return
	end
	...
	if item.type == "item_weapon" then
		...
		local sourceInventory <const> = UsersInventories.default[sourceCharacter.identifier]
		local sourceItem <const> = (sourceInventory and item and item.id) and sourceInventory[item.id] or nil
		if not sourceItem then
			SvUtils.Trem(_source)
			return
		end
		...
		local res = InventoryAPI.canCarryItem(target, itemName, amount)
		if not res then
			SvUtils.Trem(_source)
			return Core.NotifyObjective(_source, T.cantCarryItemStack, 2000)
		end

		InventoryAPI.subItemID(_source, sourceItem:getId(), function(removed)
			if not removed then
				SvUtils.Trem(_source)
				return
			end

			InventoryAPI.addItem(target, itemName, amount, itemMeta, function(added)
				if not added then
					InventoryAPI.addItem(_source, itemName, amount, itemMeta, function()
						SvUtils.Trem(_source)
					end, true, itemDegradation, itemPercentage)
					return
				end

				SetTimeout(400, function()
					InventoryService.reloadInventory(target, "default", "player", _source)
					InventoryService.DiscordLogs(invId, itemName, amount, sourceName, "Move")
					local metadataLabel = itemMeta?.label or sourceItem:getLabel()
...
function InventoryService.TakeFromPlayer(obj)
	local _source = source
	if not rateLimit(_source, "TakeFromPlayer", 150) then
		return
	end
	local data = safeJsonDecode(obj)
	if type(data) ~= "table" then
	...
	if item.type == "item_weapon" then
		...
		local targetCharacter <const> = targetUser.getUsedCharacter
		local targetInventory <const> = UsersInventories.default[targetCharacter.identifier]
		local targetItem <const> = (targetInventory and item and item.id) and targetInventory[item.id] or nil
		if not targetItem then
			SvUtils.Trem(_source)
			return
		end

		if amount > targetItem:getCount() then
			SvUtils.Trem(_source)
			return Core.NotifyObjective(_source, T.notEnoughItems, 2000)
		end

		local itemName <const> = targetItem:getName()
		local itemMeta <const> = targetItem:getMetadata()
		local itemDegradation <const> = targetItem:getDegradation()
		local itemPercentage <const> = targetItem:getPercentage()

		local res = InventoryAPI.canCarryItem(_source, itemName, amount)
		if not res then
			SvUtils.Trem(_source)
			return Core.NotifyObjective(_source, T.cantCarryItemStack, 2000)
		end

		InventoryAPI.subItemID(target, targetItem:getId(), function(removed)
			if not removed then
				SvUtils.Trem(_source)
				return
			end

			InventoryAPI.addItem(_source, itemName, amount, itemMeta, function(added)
				if not added then
					InventoryAPI.addItem(target, itemName, amount, itemMeta, function()
						SvUtils.Trem(_source)
					end, true, itemDegradation, itemPercentage)
					return
				end
				...
				InventoryService.reloadInventory(target, "default", "player", _source)
				InventoryService.DiscordLogs(invId, itemName, amount, sourceName, "Take")
				local metadataLabel = itemMeta?.label or targetItem:getLabel()
